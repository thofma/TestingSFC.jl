...
2. Run
```julia-repl
julia> using Pkg; pkg"registry add https://github.com/thofma/Registry"; pkg"up"; pkg"add TestingSFC, MagmaGroups";
```
...
> [!WARNING]  
> All positive results, where $\mathbf{Z}[G]$ is claimed to have stably free cancellation, are subject to the condition that some (and therefore every) maximal order containing $\mathbf{Z}[G]$ the has stably free cancellation. This is not checked by the algorithm. For the examples in the README and the paper, this is guaranteed by theory.
